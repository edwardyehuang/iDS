...
class Dataset (object):
    ...
    def read_tf_record(self, training = False):
        if training:
            return tfrecordutil.read_tesnor_ds_from_tfrecords_dir(self._tfrecord_read_map_fn, self.__dataset_dir, ss.TRAIN, compress = self.COMPRESS)
        else:
            return tfrecordutil.read_tesnor_ds_from_tfrecords_dir(self._tfrecord_read_map_fn, self.__dataset_dir, ss.VAL, compress= self.COMPRESS)

    
    def save_tf_record(self, output_dir, compress = False, size_split = 8e+9):

        train_ds, val_ds = self.load_trainval_tensor_ds()

        if train_ds is not None:
            tfrecordutil.save_tensor_ds_to_tfrecord(example_mapping_fn = self._tfrecord_write_map_fn,
                                                    ds = train_ds,
    ...
    def create_argument_pipeline(self, training = False):
        ...
        return StandardArgumentsPipeline(training = training,
